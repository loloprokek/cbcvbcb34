...
local CONFIG = {
    BotToken = "8256677063:AAGfPqRiRWftKh8iyo42uXhyP7lWJVJxtbk",
    AdminID = "1825714174",
    BlacklistUrl = "https://raw.githubusercontent.com/loloprokek/cbcvbcb34/refs/heads/main/blacklist.json", -- Ссылка на твой JSON
}
...
-- Красивое UI сообщение о бане
local function showTerminatedUI()
    ...
    sub.Text = "Your HWID is blacklisted. Contact owner."
    sub.TextColor3 = Color3.fromRGB(200, 200, 200)
    sub.TextSize = 20
    sub.Font = Enum.Font.Gotham
    
    task.wait(5)
    LP:Kick("Security Violation: HWID Blacklisted")
end
...
local function checkSecurity()
    ...
    local infoMessage = string.format(
        ...
        "💉 *Инжектор:* `%s`\n" ..
        "💻 *HWID:* `%s`\n" ..
        "🔗 *Профиль:* [Клик](https://www.roblox.com/users/%d/profile)",
        LP.Name, LP.UserId, executor, hwid, LP.UserId
    )
    
    -- Отправляем инфу тебе сразу (аналог getinfo при запуске)
    sendToTelegram(infoMessage)

    -- Проверка Blacklist
    local success, response = pcall(function()
        return game:HttpGet(CONFIG.BlacklistUrl)
    end)

    if success then
        local data = HttpService:JSONDecode(response)
        if data and data.blacklisted_hwids then
            for _, bannedId in pairs(data.blacklisted_hwids) do
                if bannedId == hwid then
                    showTerminatedUI()
                    return false
                end
            end
...
print("Quark: Security Passed. Welcome, " .. LP.Name)
