...
if not serverHopFile or not serverHopData.timestamp or (tick() - serverHopData.timestamp) > 3600 then
    print("Resetting server hop data.")
    serverHopData = {
        ["cursor"] = "",
        ["visited"] = {},
        ["timestamp"] = tick()
...
local function TPReturner()
    ...
    if not success or not Site or not Site.data then
        print("Failed to get server list or list is empty. Waiting 5 seconds...")
        serverHopData.cursor = "" 
        serverHopData.visited = {}
        task.wait(5) -- ИЗМЕНЕНИЕ: Добавлена задержка при ошибке получения списка серверов
        return
    end
    ...
    if nextPageCursor and nextPageCursor ~= "null" and nextPageCursor ~= nil then
        ...
        print("Reached end of server list, resetting.")
        serverHopData.cursor = ""
        serverHopData.visited = {}
    end
    writefile("AutoPres3_ServerHop.txt", game:GetService('HttpService'):JSONEncode(serverHopData))
...
local function Teleport()
    ...
    -- ИЗМЕНЕНИЕ: Увеличен интервал между сканированием страниц серверов
    while task.wait(10) do 
        TPReturner()
        print("Finished a server page scan. Trying next page or resetting...")
    end
end
